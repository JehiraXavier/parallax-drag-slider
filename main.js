$(document).ready(function () {
    ...
    let generate_slide = function (city) {
      ...
      const letter =
      $(`<div class="slide_letter slide-${num_slide}_letter">
                              ${first_letter}
                          </div>`);
  
      for (let i = 0, length = cities_divided[city].length; i < length; i++) {
        const text =
        $(`<div class="slide_text slide_text-${i + 1}">
                                  ${cities_divided[city][i]}
                              </div>`);
        fragment_1.append(text);
      }
  
      const nav_slide = $(`<li data-target="${num_slide}" class="nav_slide nav_slide-${num_slide}"></li>`);
      fragment_2.append(nav_slide);
      $nav.append(fragment_2);
  
      $slide.find(`.slide-${num_slide}_text-wrapper`).append(letter).append(fragment_1);
      $slider.append($slide);
  
      if (array_cities[city].length <= 4) {
        $('.slide-' + num_slide).find('.slide_text').css("font-size", "12vw");
      }
    };
    ...
    $(document).on('mousedown touchstart', '.slide', function (e) {
      ...
      $(document).on('mousemove touchmove', function (e) {
        ...
        diff = startX - x;
        if (target === 1 && diff < 0 || target === cities_no && diff > 0) return;
  
        $slider.css({
          'transform': 'translate3d(-' + ((cur_slide - 1) * 100 + diff / 30) + '%, 0, 0)' });
  
  
        $slider.find('.slide_dark_background').css({
          'transform': 'translate3d(' + ((cur_slide - 1) * 50 + diff / 60) + '%, 0, 0)' });
